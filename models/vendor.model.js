...
vendorInvitationSchema.statics.createInvitation = async function(data, adminId) {
...
vendorSchema.statics.createFromInvitation = async function(invitation, userId) {
  return this.create({
    user: userId,
    status: 'active',
    joinedDate: new Date()
...
vendorSchema.methods.updateStats = async function() {
  ...
  const products = await mongoose.model('Product').countDocuments({ vendor: this._id });
...
const VendorInvitation = mongoose.model('VendorInvitation', vendorInvitationSchema);
